...
def daily_cashflow(sales, investor_exit, report_date):
    ...
    for i in range(5, ws1.max_row + 1):
        ...
        # "=IF(H67>=SUMIFS(Investors!$P:$P,Investors!$G:$G,Sales!C67),SUMIFS(Investors!$R:$R,Investors!$G:$G,Sales!C67),0)"
        ws1[f"R{i}"] = f"=IF(H{i}>=SUMIFS(Investors!$P:$P,Investors!$G:$G,Sales!C{i}),SUMIFS(Investors!$R:$R,Investors!$G:$G,Sales!C{i}),0)"
        # "=Q5-R5"
        ws1[f"S{i}"] = f"=Q{i}-R{i}"
